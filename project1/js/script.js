...
$('#btnGoCountry').click(function() {
    ...
    getAllCountries().then(countriesData => {
        ...
        getCountryData(countryNumber).then(data => {
            ...
            getInformation(data.properties.iso_a2, feature)
            feature.addTo(myMap)
            return bounds
        }).then(bounds => {
            getWeather(bounds)
...
const getWeather = (bounds) => {
    ...
    var howMany = 2*parseInt(Math.sqrt((north - south)+(east-west)))
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result.status.name == "ok") {
                ...
                myMap.on('zoomend', function() {
                    if (myMap.getZoom() <6){
                            myMap.removeLayer(tempMarkers);
                    }
                    else {
                            myMap.addLayer(tempMarkers);
                        }
                });
                ...
                for (i = 0; i < len; i++) {
                    ...
                    var latLng = L.latLng([weatherData.lat, weatherData.lng])

                    var tempMarker = L.marker(latLng, {icon: temperatureIcon(weatherData.temperature)})
                    tempMarkers.addLayer(tempMarker)

                    var windSpeedMarker = L.marker(latLng, {icon: windSpeedIcon(weatherData.windSpeed)})
                    windMarkers.addLayer(windSpeedMarker)

                    var windDirectionMarker = L.marker(latLng, {icon: windDirectionIcon(weatherData.windSpeed, weatherData.windDirection)})
                    windMarkers.addLayer(windDirectionMarker)
        ...
        success: function(result) {
            ...
            if (result.status.name == "ok") {
                ...
                var infoMarker = L.marker(latLng, {
                    icon: popIcon(data.countryName, data.continentName, data.population, data.capital, data.currencyCode, data.areaInSqKm),
                    zIndexOffset: 1000,
                    draggable: true
                })
                infoMarker.addTo(myMap)
...
$(document).ready(function() {
    $("#country").submit(function() { return false; });
});
...
const popIcon = (countryName, continentName, population, capital, currencyCode, areaInSqKm) => {
    ...
    html: `<h1 class="countryName">${countryName}</h1> <h2> Continent: ${continentName}</h2>  <h2> Population: ${population} </h2>  <h2> Capital: ${capital} </h2>  <h2> Currency: ${currencyCode} </h2>  <h2> Area: ${areaInSqKm}m<sup>2</sup> </h2>`,
    className: "mapInfo",
    iconSize: [50, 40],
    iconAnchor: [25, 10]
    })
...
const temperatureIcon = (temperature) => {
    ...
    var tempIcon = L.divIcon({
        html: `<p style="color:${colour};">${temperature}&#176 C</p>`,
...
const windSpeedIcon = (windSpeed) => {
    ...
    var windIcon = L.divIcon({
        html: `<p style="color:${colour};};">${windSpeed}kts</p>`,
        className: "windIcon",
        iconSize: [10, 10],
        iconAnchor: [-24, -10]
    })
...
const windDirectionIcon = (windSpeed, windDirection) => {
    switch (true) {
        case (windSpeed <= 0):
            colour = "#d6d6d6"
            break
    ...
    var windIcon = L.divIcon({
        html: `<p style="color:${colour};transform: rotate(${windDirection}deg);">&#8595;<p>`,
        className: "windIcon",
        iconSize: [10, 10],
        iconAnchor: [-5, -10]
    })
